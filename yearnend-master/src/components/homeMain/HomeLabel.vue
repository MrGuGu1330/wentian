...
export default {
  ...
  methods: {
    ...
    positionAll() {
      this.$nextTick(() => {
        ...
        for (let i = 1; i < max + 1; i++) {
          ...
          this.oA[i - 1].style.left =
            this.mcList[i - 1].cx +
            this.oList.offsetWidth / 2 -
            this.mcList[i - 1].offsetWidth / 2 +
            "px";
          this.oA[i - 1].style.top =
            this.mcList[i - 1].cy +
            this.oList.offsetHeight / 2 -
    ...
    update() {
      this.$nextTick(() => {
        ...
        for (let j = 0; j < this.mcList.length; j++) {
          ...
          const ry1 =
            this.mcList[j].cy * this.ca + this.mcList[j].cz * -this.sa;
    ...
    query() {
      // 假装从接口拿回来的数据
      while (this.lableNames.length < 25) {
        this.lableNames.push(
          this.lableName[Math.floor(Math.random() * this.lableName.length)]
        );
      }
      const arr = this.lableNames.map((item) => ({
        name: item,
        url: "www.baidu.com",
      }));
      const tagListOrg = arr;
      // 给tagList添加随机颜色
    ...
    onReady() {
      this.$nextTick(() => {
        ...
        this.oList.onmousemove = (event) => {
          ...
          this.mouseX =
            oEvent.clientX -
            (this.oList.offsetLeft + this.oList.offsetWidth / 2);
          this.mouseY =
            oEvent.clientY -
            (this.oList.offsetTop + this.oList.offsetHeight / 2);
          this.mouseX /= 5;
          this.mouseY /= 5;
        };
        setInterval(() => {
