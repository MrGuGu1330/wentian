<template>
  <div>
    ...
    <vxe-table ref="xTable" round border :data="tableData">

      <vxe-column type="seq" width="60"></vxe-column>
      <vxe-column title="封面图">
        <template slot-scope="scope">
          <img :src="$urlBase + '/uploadNew/' + scope.row.cover" width="100px" alt="">
        </template>
      </vxe-column>
      <vxe-column field="title" title="标题"></vxe-column>
      <vxe-column field="source" title="来源"></vxe-column>
      <vxe-column field="releaseTime" title="发布时间">
        <template slot-scope="scope">
          <i class="el-icon-time"></i>
          <span style="margin-left: 10px">{{
                scope.row.releaseTime | format
              }}</span>
        </template>
      </vxe-column>
      <vxe-column title="发布状态" show-overflow>
        <template slot-scope="scope">
          <div class="title">
            <el-tag :type="scope.row.state==='0' ? 'danger' : 'success'"> {{ scope.row.state==='0'  ? '未发布':'已发布'}}
            </el-tag>
          </div>
        </template>
      </vxe-column>
      <vxe-column field="state" title="操作" show-overflow>
        <template slot-scope="scope">
    ...
    <!--  -->
    <div class="page">
      <el-pagination background :hide-on-single-page="this.DataCount < 4" :total="this.DataCount"
        layout="total, prev, pager, next" :page-size="5" ref="page" @current-change="handleCurrentChange">
      </el-pagination>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    handeEdit (row) {
      this.$router.push(`/home/pressreleas?id=${row._id}`);
    },
    // 当前页
    handleCurrentChange (page) {
      ...
      // 分页
      queryAllnews(page, where).then((res) => {
        this.tableData = res.data.info.data;
        this.DataCount = res.data.info.count;
      });
    },
    ...
    handleDelClick (row) {
      ...
      // // 匹配src属性
      const srcReg = /src=[\\"]?([^\\"]*)[\\"]?/i;
      const arr = row.Publisher.match(imgReg);
      const imgs = [];
      if (arr) {
